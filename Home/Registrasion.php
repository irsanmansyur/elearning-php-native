    ...
    <div class="container-login100" style="background-image: url('../vendor/login/images/');">
      <div class="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-54">
        ...
        window.location.replace('?pages=registration');
            ...
            if ($level == 1) {
              $result = mysqli_query($con, "select * from tb_guru where nik='$nis' OR email='$email'");
                ...
                echo "
                  <script type='text/javascript'>
                    setTimeout(function () {
                        swal({
                          title: 'Error',
                          text:  'NIP atau Email Guru Sudah Terdaftar',
                          type: 'error',
                          timer: 3000,
                          showConfirmButton: true
                        });     
              ...
              $result = mysqli_query($con, "select * from tb_siswa where nis='$nis'");
              $numRows = mysqli_num_rows($result);
              if ($numRows >= 1) {
                echo "
                  <script type='text/javascript'>
                    setTimeout(function () {
                        swal({
                          title: 'Error',
                          text:  'NIS Siswa Sudah Terdaftar',
                          type: 'error',
                          timer: 3000,
                          showConfirmButton: true
                        });     
                ...
                $sqlSiswa = mysqli_query($con, "INSERT INTO tb_siswa VALUES(NULL,'$no','$namaUser','$_POST[jk]','$password','off','N','0','default.png','$_POST[kelas]','$_POST[jurusan]','No') ") or die(mysqli_error($con));
                if ($sqlSiswa) {
                  echo "
                <script type='text/javascript'>
                  setTimeout(function () {
